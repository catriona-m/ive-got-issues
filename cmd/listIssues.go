...
func init() {
	...
	listIssuesCmd.Flags().StringSliceP("labels", "l", []string{}, "Labels to filter issues on")
	listIssuesCmd.Flags().StringP("content", "c", "", "Filter which issues to list based on whether the content body matches input string.")
	...
	listIssuesCmd.Flags().BoolP("open-issues", "", false, "Open a browser tab for each issue found. Defaults to false.")
	listIssuesCmd.Flags().BoolP("open-prs", "", false, "Open a browser tab for each linked PR found. Must be used in conjunction with 'linked-prs'. Defaults to false.")
	listIssuesCmd.Flags().IntP("batch", "b", 0, "Specify a number of issues to list at a time. If set to 0, all issues are listed. This setting is recommended when using open-issues/prs. Defaults to 0.")
